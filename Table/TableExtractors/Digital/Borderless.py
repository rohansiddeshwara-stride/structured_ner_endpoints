...
class Borderless_Table_digital():
  def __init__(self,table_bbox,page_no,document):
    ...
    logging.info("Document loaded sucessfully")
    self.page_no =page_no
    self.page = self.doc.load_page(page_no)
    self.text_instances = self.page.get_text("words")
    logging.info(f"Page no-{page_no} read sucessfully")
  ...
  def execute(self,page_no,table_no):
    ...
    if len(self.bounding_boxes)>0:
      # print("before", self.bbox)
      x0 = min(self.bounding_boxes,key = lambda x: x[0])[0] - 5
      y0 = min(self.bounding_boxes,key = lambda x: x[1])[1] - 5
      x1 = max(self.bounding_boxes,key = lambda x: x[2])[2] + 5
      y1 = max(self.bounding_boxes,key = lambda x: x[3])[3] + 3
      self.bbox = [x0,y0, x1, y1]
      self.table_rect = self.bbox
    ...
    self.row_range = self.get_row_range(self.rows)
    logging.info("Row extraction successful")

    self.no_columns=0
    self.no_columns = self.get_no_of_columns(self.rows)
    self.column_range = self.get_column_range(self.rows,self.no_columns)
    self.columns =self.get_columns(self.all_cells,self.column_range)
    logging.info("Column extraction successful")

    self.table_with_cell_bbox, self.table_without_cell_bbox = self.create_final_table(self.rows,self.columns)
    logging.info("Table extraction successful")
    ...
    table_extracted_no_bbox = self.generate_json_old(self.table_without_cell_bbox,page_no,self.bbox)
    logging.info("extracted table successfully")
    return table_extracted_no_bbox,table_extracted
  ...
  def get_row_range(self, rows):
    # print(rows)
    row_range = []
    for r in rows:
      y0 = [x[1] for x in r]
      y1 = [x[3] for x in r]
      row_range.append([sum(y0)/len(y0), sum(y1)/len(y1)])

    new_row_positions = [row_range[0][0]]
    for i in range(len(row_range)-1):
      avg = abs(row_range[i + 1][0] - row_range[i][1])/2
      new_range = row_range[i][1] + avg
      new_row_positions.append(new_range)

    new_row_positions.append(row_range[-1][1])

    new_row_range = [[new_row_positions[i], new_row_positions[i + 1]] for i in range(len(new_row_positions) - 1)]

    return new_row_range
  ...
  def generate_json_old(self,table,page_no,bbox):
  ...
  def create_final_table(self,row_table,column_table):
    ...
    for i in text:
      ...
      if (row>=0 and col>=0):
        ...
        if self.table_with_bbox[row][col]["cell_bbox"] != [-1, -1, -1, -1]:
          new_cell_data = self.table_with_bbox[row][col]["cell_data"] + " " + i[4]
          new_cell_bbox = [self.table_with_bbox[row][col]["cell_bbox"][0], self.table_with_bbox[row][col]["cell_bbox"][1],int(i[2]),int(i[3])]
          
          self.table_with_bbox[row][col]={"cell_data": new_cell_data, 
                                        "cell_bbox":new_cell_bbox}
        else:
          self.table_with_bbox[row][col]={"cell_data": i[4], 
                                        "cell_bbox":[int(x) for x in i[0:4]]}

    # print(row_len, col_len)

    # for r in range(row_len):
    #   for c in range(col_len):
    #     cell = self.table_with_bbox[r][c]
    #     # print()
    #     if cell["cell_bbox"] == [-1, -1, -1, -1]:
    #       # print("True")
    #       self.table_with_bbox[r][c]["cell_bbox"] = [self.column_range[c][0] + 10, self.row_range[r][0], self.column_range[c][1] - 10, self.row_range[r][1] ]
  ...
  def generate_json(self,table,page_no,bbox):
    ...
    for row_id ,row in enumerate(table):
      ...
      for cell_id,cell in enumerate(row):
        cell_json = {"cell_bbox" : cell["cell_bbox"], "cell_id":str(row_id)+"_"+str(cell_id) ,"cell_value":cell["cell_data"],"cell_pageno": self.page_no}
