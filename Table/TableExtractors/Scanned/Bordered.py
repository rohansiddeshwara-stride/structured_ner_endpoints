...
class ExtractDataBorderedScanned():
    ...
    def execute(self):
        ...
        # return(self.table)
        table_extracted_without_bbox=self.generate_json_old(self.table,self.page_no,self.bbox)
        table_extracted_with_bbox=self.generate_json(self.table_with_bbox,self.page_no,self.bbox)
        return table_extracted_without_bbox,table_extracted_with_bbox
    ...
    def get_data(self):
      ...
      self.table_with_bbox=[[{"cell_data":" ","cell_bbox":[-1,-1,-1,-1]} for _ in range(col)] for _ in range(row)]
      ...
      for r in range(row - 1):
        for c in range(col - 1):
          ...
          text = results_from_ocr
          
          if self.table_with_bbox[r][r]["cell_bbox"] != [-1, -1, -1, -1]:
            new_cell_data = self.table_with_bbox[r][c]["cell_data"] + " " + text
            new_cell_bbox = [min(self.table_with_bbox[r][c]["cell_bbox"][0],x0), min(self.table_with_bbox[r][c]["cell_bbox"][1],y0),max(self.table_with_bbox[r][c]["cell_bbox"][2],x1),max(self.table_with_bbox[r][c]["cell_bbox"][3],y1)]
          
            self.table_with_bbox[r][c]={"cell_data": new_cell_data, 
                                        "cell_bbox":new_cell_bbox}
          else :
            self.table_with_bbox[r][c]={"cell_data": text, "cell_bbox":[x0, y0, x1, y1]}          
